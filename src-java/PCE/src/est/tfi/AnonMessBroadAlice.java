	...
	private class InnerTask extends SwingWorker<Void, String> {
		...
		protected Void doInBackground() throws Exception {
			try {
				...
				// esperar conexión de parte de Carol
				...
				publish("Número aleatorio recibido: " + randomStrTercero);
				...
				boolean coincidencia_bob = tcpAnt.recibirString().equals("COINCIDENCIA");
				boolean coincidencia_carol = tcpAnt.recibirString().equals("COINCIDENCIA");

				// coincidencia / diferencia
				boolean coincidencia = (monedaAliceCara && monedaCarolCara) || (!monedaAliceCara && !monedaCarolCara);
				publish("Dados los resultados (monedas) se informaría: " + (coincidencia ? "COINCIDENCIA." : "DIFERENCIA." ));
				boolean coincidencia_enviar = chkPaga.isSelected() ? !coincidencia : coincidencia;
				publish("Considerando la selección de pago se informará: " + (coincidencia_enviar ? "COINCIDENCIA." : "DIFERENCIA." ));

				// calcular total
				int totalDiferencias = 0;
				if (!coincidencia_enviar)	{
					totalDiferencias++;
				}
				if (!coincidencia_bob)	{
					totalDiferencias++;
				}
				if (!coincidencia_carol)	{
					totalDiferencias++;
				}
				publish("El total de diferencias informadas resultó de : " + totalDiferencias);
				if (totalDiferencias == 1 || totalDiferencias == 3)	{
					publish("Por lo tanto, uno de los comenzales pagará la cena.");
				} else	{
					publish("Por lo tanto, la NSA pagará la cena.");
				}

	...
	private class InnerTask extends SwingWorker<Void, String> {
		...
		protected Void doInBackground() throws Exception {
			try {
				...
				publish("Salario sumado a número aleatorio: "
						+ randomPlusSalarioStr);

				// conectar con Bob
				publish("Conectando con Bob para obtener su llave pública...");
				...
				// cifrar número con llave pública de Bob
				...
				String cifradoStr = new String(Base64.encode(cifradoBytes),
						"ISO-8859-1");
				publish("Texto cifrado (número + salario) con llave pública de Bob: "
						+ cifradoStr);
				tcp.enviarString(cifradoStr);
				publish("Texto cifrado enviado a Bob.");

				// esperar conexión (y pedido de llave) de parte de Dave
				publish("Esperando conexión de Dave...");
				...
				// recibir texto cifrado, descifrar número y calcular promedio
				...
				publish("Número recibido menos número aleatorio dividido cantidad de partes (4): $ "
						+ numResultadoBI);

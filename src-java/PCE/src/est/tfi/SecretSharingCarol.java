...
public class SecretSharingCarol extends JFrame {
	...
	public SecretSharingCarol() {
		...
		setResizable(false);
		// título
		setTitle(Principal.appName + " - Secret Sharing como Carol");
		...
		protected Void doInBackground() throws Exception {
			try {
				...
				if (seleccion.equals("esperar")) {
					...
					publish("Se esperará a que " + recv_2daParteNombre + " o "
							+ recv_3raParteNombre
					...
					String xComparte = tcp2.recibirString();
					String fxComparte = tcp2.recibirString();
					tcp2.desconectar();
					publish("Información recibida de parte de " + quienComparte
							+ ": x = " + xComparte + ", f(x) = " + fxComparte);
					
					// resolución de dos ecuaciones con dos incógnitas, o en
					...
					// pueden valer [1,3]), si la multiplicación no resuelve el
					// medio restante, toBigIntegerExact() tirará exception
						...
						if (!str.startsWith("+++")) {
							...
							if (str.equals("+++SELECCION")) {
								...
								int n = JOptionPane.showOptionDialog(_frame,
										"¿Cómo reconstruir el secreto?", "TFI - Selección",

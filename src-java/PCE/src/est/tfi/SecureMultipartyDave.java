...
public class SecureMultipartyDave extends JFrame {
	...
	public SecureMultipartyDave() {
		...
		setResizable(false);
		// título
		setTitle("TFI - PCE - Secure Multiparty Computation como Dave");
		...
		lblSalario.setBounds(10, 102, 360, 14);
		...
		tfSalario.setBounds(380, 98, 70, 23);
	...
	private class InnerTask extends SwingWorker<Void, String> {
		...
		protected Void doInBackground() throws Exception {
			try {
				...
				// esperar conexión (y pedido de llave) de parte de Carol
				publish("Esperando conexión de Carol...");
				...
				// recibir texto cifrado, descifrar número y sumar salario
				publish("Esperando número a computar...");
				...
				publish("Número descifrado: " + numStr);
				byte[] numBytes = Principal.hexStringToByteArray(numStr);
				BigInteger numBI = new BigInteger(1, numBytes);
				BigInteger numEnviar = numBI.add(new BigInteger(tfSalario
						.getText()));
				String numEnviarStr = String.format("%032x", numEnviar);
				publish("Número recibido sumado a salario: " + numEnviarStr);

				// conectar con Alice para pedirle su llave pública
				publish("Conectando con Alice para obtener su llave pública...");
				...
				publish("Llave pública obtenida:\n" + llavePubOtro.toString());

				// cifrar número con llave pública de Alice
				Cipher cipher = Cipher.getInstance("RSA", "BC");
				cipher.init(Cipher.ENCRYPT_MODE, llavePubOtro);
				byte[] cifradoBytes = cipher.doFinal(numEnviarStr
						.getBytes("ISO-8859-1"));
				String cifradoStr = new String(Base64.encode(cifradoBytes),
						"ISO-8859-1");
				publish("Texto cifrado (número + salario) con llave pública de Alice: "
						+ cifradoStr);
				tcp.enviarString(cifradoStr);
				publish("Texto cifrado enviado a Alice.");
